const COLOR_MAP = {
  ...
  lime: { text: "text-lime", bg: "bg-lime", glow: "shadow-glow-sun", ring: "ring-lime" },
};
...
export default function CabinetCard({ game, focused, onFocus, onSelect, comingSoon }) {
  ...
  return (
    <button
      ...
      className={`group relative w-full text-left rounded-cabinet border-4 p-4 sm:p-5 transition-all duration-150 outline-none
        ...
        ${focused ? `border-white/70 ${c.glow} -translate-y-1 scale-[1.03]` : "hover:-translate-y-0.5"}
      `}
    >
      {focused && (
